...
- [ ] 1. Extend CalibratedCoefficients interface and version loading
  - [ ] 1.1 Add `RegressionBlock` type and optional `regression8` field to `CalibratedCoefficients` in `src/utils/smplRegressor.ts`
    ...
    - _Requirements: 8.4_
  - [ ] 1.2 Update `loadCalibratedCoefficients()` to accept both v1.0.0 and v2.0.0
  ...
  - [ ] 1.3 Write unit tests for version loading logic
...
- [ ] 2. Implement 8-input regression path in `lookupRegress()`
  - [ ] 2.1 Add `imputeMissingMeasurements()` helper function
  ...
  - [ ] 2.2 Add `regress8Input()` internal function
  ...
  - [ ] 2.3 Add three-way routing logic in `lookupRegress()`
  ...
  - [ ] 2.4 Write unit tests for 8-input regression path
...
- [ ] 3. Modify `computeSmplBetas()` to skip refinement for full 8-input
  - [ ] 3.1 Add refinement skip logic in `computeSmplBetas()`
  ...
  - [ ] 3.2 Write unit tests for refinement skip
...
- [ ] 4. Checkpoint — Verify TypeScript changes compile and existing tests pass
...
- [ ] 5. Extend training script for dual-model export
  - [ ] 5.1 Add 8-input feature computation to `train-beta-coefficients.py`
    ...
    - _Requirements: 1.1_
  - [ ] 5.2 Add `train_regression_8input()` function
  ...
  - [ ] 5.3 Update `export_coefficients()` to include `regression8` key
  ...
  - [ ] 5.4 Update `main()` to train and export both models
...
- [ ] 6. Run training script and generate extended coefficient table
...
- [ ] 7. Checkpoint — Verify extended coefficient table loads correctly
...
- [ ] 8. Add 8-input validation test suite
  - [ ] 8.1 Extend `smplRegressor.validation.test.ts` with 8-input validation
  ...
  - [ ] 8.2 Add comparison report test
...
- [ ] 9. Add 8-input round-trip property-based tests
  - [ ] 9.1 [PBT] Write property test: 8-input round-trip chest within 5cm
  ...
  - [ ] 9.2 [PBT] Write property test: 8-input round-trip waist within 5cm
    - Same setup as 9.1, assert |extracted.waistCm - input.waistCm| < 5cm
    - _Requirements: 6.1, 6.3_
  - [ ] 9.3 [PBT] Write property test: 8-input round-trip hip within 5cm
    - Same setup as 9.1, assert |extracted.hipCm - input.hipCm| < 5cm
    - _Requirements: 6.1, 6.4_
  - [ ] 9.4 [PBT] Write property test: 8-input round-trip inseam within 5cm
    - Same setup as 9.1, assert |extracted.inseamCm - input.inseamCm| < 5cm
    - _Requirements: 6.1, 6.5_

- [ ] 10. Add structural and backward-compatibility property tests
  - [ ] 10.1 [PBT] Write property test: 8-input beta boundedness
    ...
    - 100 iterations
    - _Requirements: 2.4_
  - [ ] 10.2 [PBT] Write property test: 4-input path identity
    ...
    - 100 iterations
    - _Requirements: 9.1_
  - [ ] 10.3 [PBT] Write property test: refinement skip for full 8-input
    ...
    - 50 iterations
    - _Requirements: 4.3_
  - [ ] 10.4 [PBT] Write property test: coefficient table parse-serialize round-trip
...
- [ ] 11. Verify backward compatibility with existing tests
  - [ ] 11.1 Verify existing property-based tests pass with v2.0.0 coefficients
...
- [ ] 13. Final checkpoint — All tests pass, 8-input regression complete
