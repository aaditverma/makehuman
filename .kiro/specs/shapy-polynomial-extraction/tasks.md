...
- [ ] 1. Create A2S TypeScript module skeleton and interfaces
  - [x] 1.1 Create `src/utils/shapyA2S.ts` with A2S interfaces and loading logic
  ...
  - [x] 1.2 Implement `expandPolynomialFeatures(normalizedInputs, degree)` function
    ...
    - _Requirements: 4.2_
  - [x] 1.3 Implement `computeA2SBetas(inputs)` function
  ...
  - [x] 1.4 Write unit tests for A2S module
...
- [ ] 2. Integrate A2S path into smplRegressor.ts
  - [x] 2.1 Add `RegressionPath` type and tracking to `smplRegressor.ts`
    ...
    - _Requirements: 5.5_
  - [x] 2.2 Add A2S routing to `lookupRegress()`
  ...
  - [x] 2.3 Add A2S refinement skip in `computeSmplBetas()`
  ...
  - [x] 2.4 Add A2S initialization to engine startup
    ...
    - _Requirements: 4.6_
  - [x] 2.5 Write unit tests for A2S integration
...
- [ ] 4. Create Python A2S coefficient extraction script
  - [x] 4.1 Create `scripts/extract-shapy-a2s.py` with SHAPY source inspection
  ...
  - [x] 4.2 Implement coefficient extraction from PyTorch checkpoint
  ...
  - [x] 4.3 Implement extraction verification
    ...
    - _Requirements: 2.5_
  - [x] 4.4 Implement validation dataset generation
    ...
    - _Requirements: 6.3_
  - [x] 4.5 Implement JSON artifact export
...
- [x] 5. Run extraction script and generate artifacts
...
- [x] 6. Update polynomial feature expansion to match SHAPY's actual implementation
...
- [x] 7. Checkpoint — Verify extraction artifacts load correctly in TypeScript
...
- [ ] 8. Add A2S validation test suite (TypeScript vs Python)
  - [x] 8.1 Create `src/utils/__tests__/shapyA2S.validation.test.ts`
  ...
  - [x] 8.2 Add round-trip accuracy comparison test
...
- [ ] 9. Add A2S property-based tests
  - [x] 9.1 [PBT] Write property test: A2S beta boundedness
  ...
  - [x] 9.2 [PBT] Write property test: polynomial feature expansion length invariant
    ...
    - 100 iterations
    - _Requirements: 4.2_
  - [x] 9.3 [PBT] Write property test: A2S coefficient artifact parse-serialize round-trip
  ...
  - [x] 9.4 [PBT] Write property test: A2S beta continuity
    ...
    - 50 iterations
    - _Requirements: 9.3_
  - [x] 9.5 [PBT] Write property test: A2S round-trip chest within 5cm
    - Generate random valid A2S inputs
...
- [ ] 10. Add backward compatibility and integration property tests
  - [x] 10.1 [PBT] Write property test: A2S refinement skip
    ...
    - 50 iterations
    - _Requirements: 5.4_
  - [x] 10.2 [PBT] Write property test: backward compatibility — 4-input path identity
...
- [ ] 11. Verify backward compatibility with existing tests
  - [x] 11.1 Verify existing property-based tests pass with A2S coefficients loaded
  ...
  - [x] 11.2 Verify existing unit tests pass
    ...
    - _Requirements: 10.5_
  - [x] 11.3 Verify heuristic fallback when no coefficient artifacts present
...
- [x] 12. Final checkpoint — All tests pass, A2S extraction complete
